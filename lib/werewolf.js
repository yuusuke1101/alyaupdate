...
async function pagi(hydro, x, data) {
    ...
    return await hydro.sendMessage(x.room, {
        text: pagii(x),
        contextInfo: {
            externalAdReply: {
        ...
        for (let i = 0; i < x.player.length; i++) {
            if (x.player[i].role === "werewolf") {
                if (x.player[i].isdead != true) {
                    ...
                    await hydro.sendMessage(x.player[i].id, {
                        text: textnya,
                        mentions: membernya,
                    });
                }
...
async function win(x, t, hydro, data) {
    ...
    if (getWinner(x.room, data).status === false || x.iswin === false) {
        ...
        return await hydro.sendMessage(sesinya, {
            text: textnya,
            contextInfo: {
                externalAdReply: {
                    title: "W E R E W O L F",
...
async function run(hydro, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, hydro, data);
            break;
        } else {
            await sleep(90000);
        }
...
async function run_vote(hydro, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, hydro, data);
            break;
...
async function run_malam(hydro, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, hydro, data);
            break;
...
async function run_pagi(hydro, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, hydro, data);
            break;
